...
usage endpoint failed or returned no usable usage data.
  ...
  <Accordion title="Runtime and storage subpaths">
    ...
    | `plugin-sdk/gateway-runtime` | Least-privilege backend `callGateway`, Gateway client, event-loop-ready client start helper, gateway CLI RPC, gateway protocol errors, and channel-status patch helpers |
